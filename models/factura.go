...
type Factura struct {
	Id               int       `orm:"column(id_factura);pk"`
	NumFactura       int       `orm:"column(num_factura);null"`
	Fecha            time.Time `orm:"column(fecha);type(date);null"`
	TotalFactura     int       `orm:"column(total_factura);null"`
	IdClienteCliente *Cliente  `orm:"column(id_cliente_cliente);rel(fk)"`
}
